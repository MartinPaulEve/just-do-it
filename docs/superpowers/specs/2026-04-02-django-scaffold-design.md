...
- Image: `traefik:v3`
- HTTP port: `${TRAEFIK_PORT:-8080}` mapped to internal :80
- HTTPS port: `${TRAEFIK_HTTPS_PORT:-8443}` mapped to internal :443
- Both entrypoints always active. Self-signed certs from `traefik/certs/`, generated by `traefik/generate-cert.sh` (runs as Traefik container entrypoint, creates cert via `openssl req` if `traefik/certs/cert.pem` does not exist).
...
1. **Traefik-level HTTPS**: Access via `TRAEFIK_HTTPS_PORT` (default 8443). Traefik terminates SSL with a self-signed cert. Django sees plain HTTP internally.
...
- **Env vars for ports**: `TRAEFIK_PORT` and `TRAEFIK_HTTPS_PORT` in `.env` or command line, both entrypoints always active, simpler than a shell wrapper script
